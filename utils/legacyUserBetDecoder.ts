...
function decodeUserBetData(pubkey: PublicKey, data: Uint8Array): NormalizedUserBet | null {
    ...
    if (data.length === OLD_USER_BET_SIZE || data.length === OLD_USER_BET_SIZE + 8) {
        // OLD FORMAT: amount (u64) + is_up (bool)
        // Sometimes accounts have padding, so check if it looks like old format
    ...
    } else if (data.length >= NEW_USER_BET_SIZE) {
